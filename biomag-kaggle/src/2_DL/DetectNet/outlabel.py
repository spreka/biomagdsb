...
def classify(caffemodel, deploy_file, image_files,
    ...
    for i, image_results in enumerate(scores):
        ...
        name= os.path.basename(image_files[i])[:-4]+'.txt'
        print '==> Image #%d' % i
        for left, top, right, bottom, confidence in image_results:
            if confidence == 0:
                continue

            output=np.append(output,[left,top,right,bottom,confidence], axis= 0)
            print '%d, %d, %d, %d, %f' % (
                int(round(left)),
                int(round(top)),
                int(round(right)),
...
if __name__ == '__main__':
    ...
    if os.path.isdir(args['image_file']):
        files = glob.glob(os.path.join(args['image_file'],'*'))

    classify(args['caffemodel'], args['deploy_file'], files,
