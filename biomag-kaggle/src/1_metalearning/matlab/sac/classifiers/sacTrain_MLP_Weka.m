function classifier = sacTrain_MLP_Weka(data, parameters)
% classifier = sacTrain_MLP_Weka(data, parameters)
% Trains a multilayer perceptron classifier using the WEKA library. A 
% Classifier that uses backpropagation to classify instances.
%
% INPUT:
...
%                   -H hiddenLayers This defines the hidden layers of the 
%                      neural network. This is a list of positive whole 
%                      numbers. 1 for each hidden layer. Comma seperated. 
%                      To have no hidden layers put a single 0 here. This 
%                      will only be used if autobuild is set. There are 
%                      also wildcard values 'a' = (attribs + classes) / 
%                      2, 'i' = attribs, 'o' = classes , 't' = attribs + 
%                      classes.
%                   -L Learning rate (amount weights are updated).
%                   -M Momentum (applied to weights during updating).
%                   -N Number of epochs to train.
%
% OUTPUT:
%  classifier       a data structure containing a sac classifier
%
% See also: sacPredict_MLP_Weka, sacTrain, sacNormalizeData
...
%     parameters = '-L 0.3 -M 0.2 -N 500 -V 0 -S 0 -E 20 -H a -I';
% end
% 
...
% %classifier.type = 'MultilayerPerceptron';
% str = mfilename();
