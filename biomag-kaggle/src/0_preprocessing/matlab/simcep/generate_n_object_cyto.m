function [ nuclei ] = generate_n_object_cyto( N, representativeFeatureVectors, objectShape, objectTexture )
...
%         objectTexture);
    n = cytoplasm([0 0], nucleiInd+1, sqrt(desiredArea/pi), objectShape,...
        objectTexture);
...
%     n2 = cytoplasm(getcoords(n), nucleiInd+1, radius, [shape, shape], struct('coords',getcoords(n), 'area',sum(shape(:))));
