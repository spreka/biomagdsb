...
OUTPUTS_DIR="$WORKFLOW_ROOT/outputsValidation"
...
python3 $MASKRCNN_SCRIPTS/segmentation.py $MASKRCNN/config/predict/presegment_val.json
if [ $? -ne 0 ]
then
...
python3 ${MASKRCNN_SCRIPTS}/segmentation.py ${MASKRCNN}/config/predict/segmentation_params_2x_val.json
if [ $? -ne 0 ]
then
...
python3 ${MASKRCNN_SCRIPTS}/segmentation.py ${MASKRCNN}/config/predict/segmentation_params_4x_val.json
if [ $? -ne 0 ]
then
   echo ERROR: "Error during maskrcnn prediction"
   exit 1
