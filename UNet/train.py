...
results_root_path = '/media/tdanka/B8703F33703EF828/lassi/results/lowepoch'
...
train_dataset = JointlyTransformedDataset(train_dataset_path, transform=tf_train)
#train_dataset = BoundingBoxDataset(train_dataset_path, transform=tf_train, padding=12, minlen=32)
val_dataset = JointlyTransformedDataset(val_dataset_path, transform=tf_train)
...
#net = torch.load('/media/tdanka/B8703F33703EF828/lassi/results/UNet_forLassi_fluo/UNet_forLassi_fluo')#UNet(3, 1)
loss = JaccardLoss()
n_epochs = 200
...
# Overnight loop
for i,sigma in enumerate([0.0, 0.5, 1.0, 0.0, 0.5, 1.5]):
    model_name = 'UNet_20epoch_sigma'+str(sigma)+"_"+str(i)
    ...
    train_dataset = JointlyTransformedDataset(train_dataset_path, transform=tf_train, sigma=sigma)
    val_dataset = JointlyTransformedDataset(val_dataset_path, transform=tf_train, sigma=sigma)
    net = UNet(3, 1)
    loss = JaccardLoss()
    n_epochs = 20
    optimizer = optim.Adam(net.parameters(), lr=1e-3)
